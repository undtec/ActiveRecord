...
Protected Class Context
	#tag Method, Flags = &h0
		Function ConnectionAdapter_Count() As Integer
		    ...
		    Var lck as new TP_ActiveRecord.P.ScopedLock(m_cs)
		    #pragma unused lck
		  #Endif
	...
	#tag Method, Flags = &h0
		Sub ConnectionAdapter_Remove(ty as Introspection.TypeInfo)
		  ...
		  if  not ty.IsSubclassOf( GetTypeInfo(TP_ActiveRecord.Base) ) and _
		    not ty.IsSubclassOf( GetTypeInfo(TP_ActiveRecord.View) ) then
	...
	#tag Method, Flags = &h0
		Sub ConnectionAdapter_Set(ty as Introspection.TypeInfo, adp as TP_ActiveRecord.DatabaseAdapter)
	...
	#tag Method, Flags = &h0
		Function TableInfo_Get(ty as Introspection.TypeInfo) As TP_ActiveRecord.P.TableInfo
		  #if TargetWeb
		    //BK 04Sept2015:  Really only needed on web apps.
		    Var lck as new TP_ActiveRecord.P.ScopedLock(m_cs)
		    #pragma unused lck
	...
	#tag Method, Flags = &h0
		Function TableInfo_List() As TP_ActiveRecord.P.TableInfo()
		  ...
		  'check the info cache and return the mapping if it exists
		  Var aro() as TP_ActiveRecord.P.TableInfo
		  if m_dictTypeTableInfo=nil then
		    return aro
		  end if
		  
		  for each sKey as string in m_dictTypeTableInfo.Keys
		    Var o as TP_ActiveRecord.P.TableInfo
	...
	#tag Method, Flags = &h0
		Sub TableInfo_Set(ty as Introspection.TypeInfo, oTableInfo as TP_ActiveRecord.P.TableInfo)
		  #if TargetWeb
		    //BK 04Sept2015:  Really only needed on web apps.
		    Var lck as new TP_ActiveRecord.P.ScopedLock(m_cs)
		    #pragma unused lck
