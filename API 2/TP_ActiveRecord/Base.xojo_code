...
Protected Class Base
	#tag Method, Flags = &h0
		Function BuildChangeLog() As TP_AuditTrail.ChangeLog()
		  var aro() as TP_AuditTrail.ChangeLog
		  ...
		  for each oFieldInfo as TP_ActiveRecord.P.FieldInfo in oTableInfo.aroField
		    ...
		    if vProperty.Type=Variant.TypeString then
		      ...
		      if vProperty.StringValue.Compare(vSavedValue.StringValue, ComparisonOptions.CaseSensitive) <> 0 then
		        var oChange as new TP_AuditTrail.ChangeLog
	...
	#tag Method, Flags = &h0
		Function Clone() As Variant
		  ...
		  for each o as Introspection.ConstructorInfo in ty.GetConstructors
		    ...
		    elseif aroParam.LastIndex=0 and _
	...
	#tag Method, Flags = &h0
		Sub Delete()
		  ...
		  Var adp as TP_ActiveRecord.DatabaseAdapter = GetDatabaseAdapter
	...
	#tag Method, Flags = &h0
		Function Load(iRecordID as Int64) As Boolean
		  ...
		  Var rs as RowSet
		  Var ada as TP_ActiveRecord.DatabaseAdapter = GetDatabaseAdapter
	...
	#tag Method, Flags = &h21
		Private Sub ReadRecord(rs as RowSet, oTableInfo as TP_ActiveRecord.P.TableInfo, byref dictSavedPropertyValue as Dictionary)
		  'Read current record out of rs into properties
		  Var dictFieldValue as new Dictionary
		  
		  for each oFieldInfo as TP_ActiveRecord.P.FieldInfo in oTableInfo.aroField
		    Var oField as DatabaseColumn = rs.Column(oFieldInfo.sFieldName)
		    ' Var oField As DatabaseField = rs.Column(oFieldInfo.sFieldName)
		    Var pi as Introspection.PropertyInfo = oFieldInfo.piFieldProperty
		    ...
		    Elseif pi.PropertyType = GetTypeInfo(DateTime) Then
		      ...
		      If v = Nil Or v = "" Then
		        vProperty=Nil
		      Else
		        Var dt As DateTime = DateTime.FromString(v.StringValue)
		        vProperty=dt
		      end if
		    ...
		    if pi.PropertyType = GetTypeInfo(DateTime) and vProperty <> nil then
		      pi.Value(self) = DateTime.FromString(vProperty.StringValue)
