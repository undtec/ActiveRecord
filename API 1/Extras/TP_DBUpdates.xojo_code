...
Protected Module TP_DBUpdates
		...
		Private Sub CreateVersionTable(toLocalDB as Database)
		  try
		      ...
		      if toLocalDB isa CubeSQLVM then
		        ...
		        toLocalDB.SQLExecuteRaiseOnError(kCreateVersionTableSQLite)
		        toLocalDB.Commit
		      ...
		      if toLocalDB isa MSSQLServerDatabase then
		        toLocalDB.SQLExecuteRaiseOnError("BEGIN TRANSACTION")
		        toLocalDB.SQLExecuteRaiseOnError(kCreateVersionTableMSSQL)
		        toLocalDB.Commit
		      ...
		      if toLocalDB isa MySQLCommunityServer then
		        ...
		        toLocalDB.SQLExecuteRaiseOnError(kCreateVersionTableMySQL)
		        toLocalDB.Commit
		      ...
		      if toLocalDB isa PostgreSQLDatabase then
		        toLocalDB.SQLExecuteRaiseOnError("START TRANSACTION")
		        toLocalDB.SQLExecuteRaiseOnError(kCreateVersionTablePostgres)
		        toLocalDB.Commit
	...
	#tag Method, Flags = &h21
		Private Function GetDBVersion(toLocalDB as Database) As Integer
		  dim sSQL as String = "SELECT Max(dbversionnumber) AS CurrentVersionCode FROM t_tp_dbversion"
		  dim rs as RecordSet = toLocalDB.SQLSelectRaiseOnError(sSQL)
	...
	#tag Method, Flags = &h21
		Private Sub SetDBVersion(toLocalDB as Database, iVersion as Integer)
		  dim sSQL as String = "INSERT INTO t_tp_dbversion(dbversionnumber) VALUES (" + Format(iVersion, "#########") + ");"
		  toLocalDB.SQLExecuteRaiseOnError(sSQL)
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub UpdateDB(toLocalDB as Database)
		  ...
		  #if TP_ActiveRecord.kConfigUseCubeDatabase or _
		    TP_ActiveRecord.kConfigUseMSSQLServer or _
		    TP_ActiveRecord.kConfigUseSQLiteDatabase then
		    toLocalDB.SQLExecute("BEGIN TRANSACTION")
		    
		  #elseif TP_ActiveRecord.kConfigUseMySQLCommunityServer or _
		        TP_ActiveRecord.kConfigUsePostgreSQLDatabase then
		    toLocalDB.SQLExecute("START TRANSACTION")
		    
		  #endif
		  
		  if toLocalDB.Error then
		    raise new TP_ActiveRecord.DatabaseException(toLocalDB.ErrorMessage)
